...
def main():
    ...
    #               lr_decay=config.lr_decay, output_embfold=config.out_emd_file)
        ...
        mgg.generate_random(config.temp_file,config.num_walks,config.walk_length,g_hin.node, g_hin.relation_dict, config.mg_type)
        m2v = Metapath2VecTrainer(config)
        print("Training")
        m2v.train()
        ...
        data_process = HAN_process(g_hin, config.mp_list)
        config.out_emd_file += 'node.txt'
